...
AST* ast_addr(AST* var) {
    return ast_unary(AST_ADDR, var);
}

AST* ast_deref(AST* ptr) {
    return ast_unary(AST_DEREF, ptr);
}
...
AST* ast_if(AST* cond, AST* then_stmt, AST* else_stmt) {
    ...
    n->third = else_stmt;  // else 문장 (없으면 NULL)
    return n;
}
...
AST* ast_return(AST* expr) {
    AST* n = ast_new(AST_RETURN);
    if (!n) return NULL;
    n->left = expr;  // 반환 expr (없으면 NULL)
    return n;
}
